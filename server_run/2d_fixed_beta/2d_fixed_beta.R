        ...
        for (cook_cutoff in cook_cutoff_levels) {
          for (ctam_rho in ctam_phos_levels) {
            ...
            results_row <- data.frame(
              ...
              robust_m_est_bias = sum(robust_m_est_bias^2),
              robust_m_est_variance = sum(robust_m_est_variance),
              robust_mm_est_bias = sum(robust_mm_est_bias^2),
              robust_mm_est_variance = sum(robust_mm_est_variance),
              robust_s_est_bias = sum(robust_s_est_bias^2),
              robust_s_est_variance = sum(robust_s_est_variance),
              ols_est_bias = sum(ols_est_bias^2),
              ols_est_variance = sum(ols_est_variance),
              ols_diag_est_bias = sum(ols_diag_est_bias^2),
              ols_diag_est_variance = sum(ols_diag_est_variance),
              ols_mspe = ols_mspe,
...
# # Save results to RDS
saveRDS(results, "2d_fixed_results.RDS")
# saveRDS(run_log, "run_log.RDS")

# # Save results to CSV, discarding the index column
write.csv(results, "2d_fixed_results.csv", row.names = FALSE)
