...
# ctam_mu_levels <- c(0)
# cook_cutoff_levels <- c(1)
# num_runs <- 100

n_list <- c(200)
contamination_levels <- seq(from = 0, to = 0.6, by = 0.1)
ctam_tdist_df_levels <- c(1, 2, 5, 10, 20)
...
generate_data <- function(n, contamination_level, ctam_df){
  ...
  if (n_contaminated > 0) {
    ...
    x[contaminated_indices] <- x[contaminated_indices] + rt(n_contaminated, df = ctam_df)
  }
...
for (n in n_list) {
  for (contamination_level in contamination_levels) {
    for (ctam_df in ctam_tdist_df_levels) {
        for (cook_cutoff in cook_cutoff_levels) {
          # show a progress message
          print(paste("n:", n, "contamination_level:", contamination_level, "ctam_df:", ctam_df, "cook_cutoff:", cook_cutoff))
          # Define metrics
          robust_m_est <- c()
          robust_mm_est <- c()
          robust_s_est <- c()
          ...
          ols_est_squared_error <- c()
          robust_m_est_squared_error <- c()
          robust_mm_est_squared_error <- c()
          robust_s_est_squared_error <- c()
          ols_diag_est_squared_error <- c()
          ...
          for (run in 1:num_runs) {
            data <- generate_data(n, contamination_level, ctam_df)
            ...
            ols_est_squared_error <- c(ols_est_squared_error, (results_list$ols_coef[2] - beta_1)^2)
            robust_m_est_squared_error <- c(robust_m_est_squared_error, (results_list$robust_fit_m$coefficients[2] - beta_1)^2)
            robust_mm_est_squared_error <- c(robust_mm_est_squared_error, (results_list$robust_fit_mm$coefficients[2] - beta_1)^2)
            robust_s_est_squared_error <- c(robust_s_est_squared_error, (results_list$robust_fit_s$coefficients[2] - beta_1)^2)
            ols_diag_est_squared_error <- c(ols_diag_est_squared_error, (results_list$ols_diag_coef[2] - beta_1)^2)
          ...
          ols_est_mse <- mean(ols_est_squared_error)
          robust_m_est_mse <- mean(robust_m_est_squared_error)
          robust_mm_est_mse <- mean(robust_mm_est_squared_error)
          robust_s_est_mse <- mean(robust_s_est_squared_error)
          ols_diag_est_mse <- mean(ols_diag_est_squared_error)
...
# # Save results to RDS
saveRDS(results, "1d_fixed_results_tdist.RDS")
# saveRDS(run_log, "run_log.RDS")

# # Save results to CSV, discarding the index column
write.csv(results, "1d_fixed_results_tdist.RDS", row.names = FALSE)
